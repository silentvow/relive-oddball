...
import { fetchMyRankingWL, fetchVtubersWL } from "@/lib/dataWL";

export const size = { width: 1200, height: 630 };
export const contentType = "image/png";
export const alt = "Wang-Low ranking card";
...
export default async function OpenGraphImage({
  ...
  const [vtubers, ranking] = await Promise.all([
    fetchVtubersWL(),
    fetchMyRankingWL(params.uuid),
  ]);
  const byId = Object.fromEntries(vtubers.map((v) => [v.id, v]));

  const top3 = (ranking ?? [])
    .slice(0, 3)
    .map((r) => byId[r.vtuberId])
    .filter(Boolean);

  const glyphs = "海亞法克你又望樓怪咖排名我的奇沒有對錯就是感覺ReLiveProjectOddballWangLow#";
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ fontSize: 60, fontWeight: 500, marginTop: 12 }}>
          我的怪咖排名
        </div>
        ...
        >
          {top3.map((v, i) => {
            const rank = i + 1;
            const ringColor =
              rank === 1 ? "#C49B8C" : rank === 2 ? "#D9BFA8" : "#DDC987";
            const sz = rank === 1 ? 200 : 160;
            return (
              <div
                key={v.id}
                style={{
        ...
        >
          <span>#ReLiveOddball #WangLow</span>
