...
// Wang-low share bar. Same UX as the main ShareBar but the share URL points
// to /wang-low/share/[uuid] and the share text mentions wang-low.
...
export function ShareBar({ uuid, ranking, byId }: Props) {
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/wang-low/share/${uuid}`
      : "";
  ...
  async function handleDownload() {
    ...
    try {
      ...
      a.href = dataUrl;
      a.download = `wang-low-${new Date().toISOString().slice(0, 10)}.png`;
      a.click();
      toast.success("圖片已下載");
    } catch (e) {
  ...
  const tweetUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(
    "誰最奇怪？這是我的海亞！法克你又望樓怪咖排名 → ",
  )}&url=${encodeURIComponent(shareUrl)}&hashtags=ReLiveOddball,WangLow`;
  const threadsUrl = `https://www.threads.net/intent/post?text=${encodeURIComponent(
    `誰最奇怪？這是我的海亞！法克你又望樓怪咖排名 →\n${shareUrl}\n#ReLiveOddball #WangLow`,
  )}`;
