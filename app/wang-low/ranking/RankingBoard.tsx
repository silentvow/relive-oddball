...
// Wang-low (7-Vtuber) ranking board. Mirrors app/ranking/RankingBoard.tsx
// almost line-for-line — slot count derived from vtubers.length (7), submit
// goes to submitRankingWL, draft uses storageWL keys, redirect points to
// /wang-low/results.
...
import { submitRankingWL } from "@/app/actionsWL";
...
function freshState(vtubers: Vtuber[]): DraftStateWL {
  return {
    pool: [...vtubers]
      .sort((a, b) => a.displayOrder - b.displayOrder)
      .map((v) => v.id),
    slots: Array(vtubers.length).fill(null),
  };
}
...
export function RankingBoard({ vtubers, submissionCount }: Props) {
  const slotCount = vtubers.length;
  const lastRank = slotCount;
  ...
  const [state, setState] = useState<DraftStateWL>(() => freshState(vtubers));
  const [draggingId, setDraggingId] = useState<number | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const router = useRouter();

  useEffect(() => {
    const draft = loadDraftWL();
    if (draft) setState(draft);
  }, []);

  const isFresh = state.slots.every((s) => s === null);
  useEffect(() => {
    if (!isFresh) saveDraftWL(state);
  }, [state, isFresh]);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await submitRankingWL({ userUuid, ranking });
      if (result.ok) {
        markSubmittedWL();
        toast.success("送出成功！");
        router.push(`/wang-low/results?u=${userUuid}`);
      } else {
